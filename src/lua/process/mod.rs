...
use mlua::{
    Function, Lua, MetaMethod, MultiValue, RegistryKey, Result as LuaResult, Table, UserData, UserDataFields,
...
tokio::task_local! {
    static PROC_MW_STACK: RefCell<ProcMiddlewareStack>;
}

struct ProcMiddlewareStack {
    lua: Lua,
    keys: Vec<RegistryKey>,
}

impl Drop for ProcMiddlewareStack {
    fn drop(&mut self) {
        // Best-effort cleanup: remove any remaining registry keys.
...
pub(crate) fn proc_middleware_snapshot(lua: &Lua) -> LuaResult<Vec<RegistryKey>> {
    // Snapshot the current task's middleware stack for inheritance into a child task.
    //
    // RegistryKey is move-only (not Clone), so we duplicate the underlying functions by
    // re-registering them and returning the newly created keys. The child scope owns these
    // keys and will clean them up on drop.
    let r = PROC_MW_STACK.try_with(|cell| {
        let st = cell.borrow();
        ...
        let mut guard = Guard {
            lua,
            keys: Vec::with_capacity(st.keys.len()),
        };

        for k in &st.keys {
            let f: Function = st.lua.registry_value(k)?;
            let nk = lua.create_registry_value(f)?;
            guard.keys.push(nk);
        }

        Ok(std::mem::take(&mut guard.keys))
    });
...
fn proc_middleware_funcs_snapshot() -> LuaResult<Vec<Function>> {
    // Build a callable snapshot of the current stack without cloning RegistryKey.
    // We must avoid holding a RefCell borrow across middleware calls (which can re-enter
    // process APIs), so we materialize a Vec<Function> and drop the borrow immediately.
    let r = PROC_MW_STACK.try_with(|cell| {
        let st = cell.borrow();
        let mut out: Vec<Function> = Vec::with_capacity(st.keys.len());
        for k in &st.keys {
            out.push(st.lua.registry_value::<Function>(k)?);
        }
        Ok(out)
    });
...
pub(crate) fn proc_middleware_scope<Fut, T>(lua: Lua, keys: Vec<RegistryKey>, fut: Fut) -> impl Future<Output = T>
where
    Fut: Future<Output = T>,
{
    PROC_MW_STACK.scope(RefCell::new(ProcMiddlewareStack { lua, keys }), fut)
}

fn with_proc_mw_stack_mut<R>(f: impl FnOnce(&mut ProcMiddlewareStack) -> LuaResult<R>) -> LuaResult<R> {
    PROC_MW_STACK
        .try_with(|cell| {
            let mut st = cell.borrow_mut();
            f(&mut st)
        })
        .map_err(|_| mlua::Error::external("process middleware context unavailable"))?
}

fn cmdspec_to_mw_table(lua: &Lua, spec: &CmdSpec) -> LuaResult<Table> {
    let t = lua.create_table()?;

    let argv = lua.create_table()?;
    argv.raw_set(1, spec.program.clone())?;
    for (i, a) in spec.args.iter().enumerate() {
        #[allow(clippy::cast_possible_wrap)]
        argv.raw_set((i + 2) as i64, a.clone())?;
    }
    t.set("argv", argv)?;

    if let Some(cwd) = &spec.cwd {
        t.set("cwd", cwd.to_string_lossy().into_owned())?;
    } else {
        t.set("cwd", Value::Nil)?;
    }
...
fn apply_mw_table_to_cmdspec(t: Table, spec: &mut CmdSpec) -> LuaResult<()> {
    // argv (array table of strings)
    if let Some(v) = t.get::<Option<Value>>("argv")? {
        match v {
            Value::Nil => {}
            Value::Table(at) => {
                let mut argv: Vec<String> = Vec::new();
                for (ix0, v) in at.sequence_values::<Value>().enumerate() {
                    match v? {
                        Value::String(s) => argv.push(s.to_str()?.to_string()),
                        other => {
                            return Err(mlua::Error::RuntimeError(format!(
                                "middleware: argv[{}] must be string, got {}",
                                ix0 + 1,
                                other.type_name()
                            )));
                        }
                ...
                if argv.is_empty() {
                    return Err(mlua::Error::RuntimeError("middleware: argv must not be empty".into()));
                }
                spec.program = argv.remove(0);
                spec.args = argv;
            }
            other => {
                return Err(mlua::Error::RuntimeError(format!(
                    "middleware: argv must be table or nil, got {}",
                    other.type_name()
                )));
            }
        }
    ...
    // cwd (string|nil)
    if let Some(v) = t.get::<Option<Value>>("cwd")? {
        match v {
            Value::Nil => {}
            Value::String(s) => {
                spec.cwd = Some(std::path::PathBuf::from(s.to_str()?.to_string()));
            }
            other => {
                return Err(mlua::Error::RuntimeError(format!(
                    "middleware: cwd must be string or nil, got {}",
                    other.type_name()
                )));
            }
        }
    ...
    // env (table) - merge
    if let Some(v) = t.get::<Option<Value>>("env")? {
        match v {
            Value::Nil => {}
            Value::Table(et) => {
                for pair in et.pairs::<Value, Value>() {
                    let (k, v) = pair?;
            ...
            other => {
                return Err(mlua::Error::RuntimeError(format!(
                    "middleware: env must be table or nil, got {}",
                    other.type_name()
                )));
            }
        }
...
fn apply_process_middlewares(lua: &Lua, spec: &mut CmdSpec) -> LuaResult<()> {
    let funcs = proc_middleware_funcs_snapshot()?;
    if funcs.is_empty() {
        return Ok(());
    }

    for f in funcs {
        let t = cmdspec_to_mw_table(lua, spec)?;

        // Middleware may either mutate the passed table and return nil,
        // or return an alternate spec table.
        let out: Value = f.call(t.clone())?;
        let out_table = match out {
            Value::Nil => t,
            Value::Table(tt) => tt,
            other => {
                return Err(mlua::Error::RuntimeError(format!(
                    "middleware must return table or nil, got {}",
                    other.type_name()
                )));
            }
        };

        apply_mw_table_to_cmdspec(out_table, spec)?;
    }
...
fn duration_to_millis(dur: Duration) -> u64 {
    let ms_total: u128 = u128::from(dur.as_secs()) * 1000 + u128::from(dur.subsec_nanos()).div_ceil(1_000_000);
    u64::try_from(ms_total.min(u128::from(u64::MAX))).unwrap_or(u64::MAX)
}
    ...
    fn add_methods<M: UserDataMethods<Self>>(methods: &mut M) {
        ...
        methods.add_async_method("run", |lua, this, ()| {
            ...
            async move {
                ...
                let mode = defaults
                    .timeout_ms
                    .map_or(RunMode::Inherit, |ms| RunMode::InheritWithTimeout(Some(ms)));
                run_specs(lua.clone(), vec![spec], defaults.pipefail, mode, overlay).await
            }
        });
        ...
        methods.add_async_method("output", |lua, this, ()| {
            ...
            async move {
                ...
                let mode = defaults
                    .timeout_ms
                    .map_or(RunMode::Capture, |ms| RunMode::CaptureWithTimeout(Some(ms)));
                run_specs(lua.clone(), vec![spec], defaults.pipefail, mode, overlay).await
            }
        });
        ...
        methods.add_async_method("spawn", |lua, this, opts: Option<Table>| {
            ...
            async move {
                ...
                let defaults = shell_defaults_snapshot();
                spawn_specs(lua.clone(), vec![spec], defaults.pipefail, defaults.timeout_ms, cfg, overlay).await
            }
        });
...
impl UserData for Pipeline {
    fn add_methods<M: UserDataMethods<Self>>(methods: &mut M) {
        ...
        methods.add_async_method("run", |lua, this, ()| {
            ...
            async move {
                let overlay = crate::lua::env::overlay_snapshot(&lua)?;
                run_specs(lua.clone(), specs, pipefail, RunMode::InheritWithTimeout(timeout_ms), overlay).await
            }
        });
        ...
        methods.add_async_method("output", |lua, this, ()| {
            ...
            async move {
                let overlay = crate::lua::env::overlay_snapshot(&lua)?;
                run_specs(lua.clone(), specs, pipefail, RunMode::CaptureWithTimeout(timeout_ms), overlay).await
            }
        });
...
pub fn define(lua: &Lua) -> LuaResult<Table> {
    ...
    m.set("push_middleware", lua.create_function(lua_push_middleware)?)?;
    m.set("pop_middleware", lua.create_function(lua_pop_middleware)?)?;
    Ok(m)
}

fn lua_push_middleware(lua: &Lua, f: Function) -> LuaResult<usize> {
    let r = PROC_MW_STACK.try_with(|cell| {
    ...
    match r {
        Ok(Ok(n)) => Ok(n),
        Ok(Err(e)) => Err(e),
        Err(_) => Err(mlua::Error::external("process middleware context unavailable")),
    }
}

fn lua_pop_middleware(_: &Lua, _: ()) -> LuaResult<bool> {
    with_proc_mw_stack_mut(|st| {
        let Some(key) = st.keys.pop() else {
            return Ok(false);
        };
        st.lua.remove_registry_value(key)?;
        Ok(true)
    })
}
