...
async fn listen(_lua: &Lua, path: PathBuf, opts: Option<Table>) -> mlua::Result<UnixListenerUserData> {
    ...
    UnixListenerUserData::new(listener, path, options.unlink_on_close, true)
}
...
impl UnixListenerUserData {
    fn new(listener: UnixListener, path: PathBuf, unlink_on_close: bool, own_path: bool) -> mlua::Result<Self> {
        let ud = Self {
