...
pub fn define(lua: &Lua) -> mlua::Result<Table> {
    ...
    t.set("platform", lua.create_function(|_, ()| Ok(format!("{}-{}", std::env::consts::OS, std::env::consts::ARCH)))?)?;

    // Runtime-ish (best-effort)
    t.set("version", lua.create_function(|_, ()| Ok(System::long_os_version().unwrap_or_default()))?)?;
    t.set("release", lua.create_function(|_, ()| Ok(System::kernel_version().unwrap_or_default()))?)?;
    t.set("hostname", lua.create_function(|_, ()| Ok(System::host_name().unwrap_or_default()))?)?;

    // Capabilities / conventions
    t.set("exe_suffix", lua.create_function(|_, ()| Ok(if cfg!(windows) { ".exe" } else { "" }))?)?;
    t.set("path_sep", lua.create_function(|_, ()| Ok(if cfg!(windows) { "\\" } else { "/" }))?)?;
    t.set("env_sep", lua.create_function(|_, ()| Ok(if cfg!(windows) { ";" } else { ":" }))?)?;
    t.set("newline", lua.create_function(|_, ()| Ok(if cfg!(windows) { "\r\n" } else { "\n" }))?)?;
    t.set("endianness", lua.create_function(|_, ()| Ok(if cfg!(target_endian = "little") { "little" } else { "big" }))?)?;

    // Default shell invocation as { prog, args_table }
    t.set("shell", lua.create_function(|lua, ()| {
    ...
    t.set("info", lua.create_function(|lua, ()| {
        ...
        info.set("endianness", if cfg!(target_endian="little") { "little" } else { "big" })?;
